...
export type TradingMode = 'normal' | 'advanced';
...
export interface BotConfig {
  ...
  tradingMode: TradingMode; // 'normal' = standard logic, 'advanced' = auto pair-swap on low win rate
  barrierDigit: number; // Barrier for DIGITOVER: 1-9. Default 4.
  dailyStopLoss: number;   // Max daily loss before bot locks (in account currency). Default 0 (disabled).
  dailyTakeProfit: number; // Max daily profit before bot locks (in account currency). Default 0 (disabled).
...
export interface BotState {
  ...
  cooldownActive: boolean;    // True when in cooldown after daily limit or loss cap
  cooldownRemaining: number;   // Seconds remaining in cooldown (0 if inactive)
