    ...
    return (
      <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 z-50">
        <motion.div
          ...
          className="w-full max-w-md bg-slate-950 border border-rose-500/20 rounded-2xl shadow-2xl overflow-hidden p-6 text-center space-y-6"
        >
          <div className="flex flex-col items-center space-y-3">
            <div className="w-16 h-16 rounded-full bg-rose-500/10 border border-rose-500/25 flex items-center justify-center text-rose-450 animate-pulse">
              <ShieldAlert className="w-8 h-8" />
            </div>
            <span className="text-[10px] font-mono uppercase bg-rose-550/10 text-rose-455 border border-rose-550/20 px-2.5 py-1 rounded-full tracking-widest font-bold">
              Stop Loss Intercepted
            </span>
            <h2 className="text-xl font-bold font-sans tracking-tight text-slate-100 uppercase">
              Autonomic Safety Disarm
            </h2>
            <p className="text-xs text-slate-400 max-w-sm">
              The automated entry module has been paused. The bot reached its consecutive loss boundary or daily stop loss limit, trigger-disarming entries to prevent further fallout.
            </p>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-900 rounded-xl p-4 grid grid-cols-3 gap-2 font-mono">
            <div className="text-center p-2 rounded bg-slate-950/50 border border-slate-900">
              <span className="text-[9px] text-slate-500 block uppercase font-bold tracking-wider">Net Loss</span>
              <span className="text-sm font-black text-rose-400 block mt-0.5">${profit.toFixed(2)}</span>
            </div>
            <div className="text-center p-2 rounded bg-slate-950/50 border border-slate-900">
          ...
          <button
            onClick={onClose}
            className="w-full cursor-pointer py-3 px-6 rounded-xl font-mono text-xs uppercase font-extrabold tracking-widest bg-rose-550 hover:bg-rose-500 text-white shadow-lg shadow-rose-950/20 transform active:scale-98 transition-all flex items-center justify-center gap-1.5"
          >
            <RefreshCw className="w-4 h-4" /> DISARM & FLUSH SESSION
          </button>
        </motion.div>
      </div>
    );
