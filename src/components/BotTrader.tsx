...
function getSymbolMetrics(state: SymbolState, barrier: number) {
  ...
  const signalFreqVal = state.ticks > 10 ? (state.signals / state.ticks) * 100 : 0;
  ...
  if (winRate !== null) {
    ...
    const freqBonus = Math.min(signalFreqVal * 2.0, 5.0);
...
export function BotTrader({
  ...
  const [credibilityInput, setCredibilityInput] = useState((botConfig.credibilityFalloutPercent || 55).toString());
  ...
  useEffect(() => { setCredibilityInput((botConfig.credibilityFalloutPercent || 55).toString()); }, [botConfig.credibilityFalloutPercent]);

  // Auto-update defaults when switching trading modes
  useEffect(() => {
    const updates: Partial<BotConfig> = { martingaleMultiplier: 2.0, maxWins: 3, maxLosses: 5 };
    onUpdateConfig(updates);
    setMartingaleInput('2.0');
    setMaxWinsInput('3');
    setMaxLossesInput('5');
  }, [isAdvancedMode]);
  useEffect(() => {
  ...
  const getBestSymbolId = () => {
    const inResume = isResumeModeRef.current; const activeBarrier = getBarrierDigit(botConfig.tradingMode, botConfig.barrierDigit);
    const ranked = Object.values(symbolsState)
      .filter(s => !inResume || s.info.id !== botState.symbol)
      .map(s => {
        const metrics = getSymbolMetrics(s, activeBarrier);
        ...
        if (winRate !== null) {
          if (winRate >= 55.0) {
            // High credibility: score ranges between 55       and 100
            score = winRate + Math.min(freq * 2.0, 4.0);
          } else {
            // Low credibility: score capped lower
            score = winRate * 0.8;
          }
          if (total < 5) {
    ...
    if (inResume) {
      const q = ranked.find(s => s.winRate !== null && s.winRate >= 55.0);
      if (q) return q.id;
    }
    ...
    const iv = setInterval(() => {
      if (idx < symbols.length * 2) { setScanIndex(idx % symbols.length); idx++; }
        ...
        if (isResumeModeRef.current) {
          ...
          const bestState = symbolsState[best];
          const bestTotal = bestState ? (bestState.wins + bestState.losses) : 0;
          const bestWinRate = bestState && bestTotal >= 3 ? (bestState.wins / bestTotal) * 100 : 0;
          if (bestWinRate >= 55.0) {
            onResumeWithSymbol(best);
          } else {
            addLocalLog('warning', `⏳ SCAN UPDATE: No alternative pair currently qualifies with ≥55% credibility score. Standing by for background simulation updates.`);
          }
        } else {
          onSelectSymbolForTrading(best, false);
        }
      }
    }, 100);
  };
    ...
    <div className="space-y-4 pb-20 font-sans">
      ...
      <div className="rounded-xl border border-slate-800/80 bg-slate-900/90 overflow-hidden shadow-lg select-none">
        ...
        <div>
          ...
          {botState.cooldownActive && botState.cooldownRemaining > 0 && (
            <div className="mx-3 mt-2.5 p-2 rounded-lg bg-amber-500/5 border border-amber-500/25 text-[10px] text-slate-300 flex items-start gap-1.5 animate-fade-in font-sans text-left">
              <RefreshCw className="w-3 h-3 text-amber-500 shrink-0 mt-0.5 animate-spin" style={{ animationDuration: '3s' }} />
              <div>
                <span className="font-bold text-slate-100 block leading-tight">Cooldown Active</span>
                Resuming in <span className="font-mono font-bold text-amber-400">{botState.cooldownRemaining}s</span>
                {botState.cooldownReason === 'daily_loss' && ' — Daily stop-loss limit reached.'}
                {botState.cooldownReason === 'daily_profit' && ' — Daily take-profit target reached.'}
                {botState.cooldownReason === 'consecutive_loss' && ' — Consecutive loss limit triggered.'}
              </div>
            </div>
          )}
        ...
        <div className="p-3 grid grid-cols-1 lg:grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`flex items-center gap-1 text-[10px] font-mono font-black px-1.5 py-0.5 rounded border transition-all disabled:opacity-40 cursor-pointer ${
                    !isAdvancedMode ? 'bg-slate-900 text-slate-300 border-slate-700' : 'bg-slate-950 text-slate-600 border-slate-800 hover:text-slate-400'
                  }`}
                >
                  <span>STANDARD</span>
                </button>
                <button
                  ...
                  disabled={botState.isRunning}
                  className={`flex items-center gap-1 text-[10px] font-mono font-black px-1.5 py-0.5 rounded border transition-all disabled:opacity-40 cursor-pointer ${
                    isAdvancedMode ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/25' : 'bg-slate-950 text-slate-600 border-slate-800 hover:text-slate-400'
                  }`}
                >
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              {isAdvancedMode && (
                <div className="space-y-0.5 text-left col-span-2">
                  <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider block">CREDIBILITY FALLOUT LIMIT</span>
                  <div className="relative">
                    <input
                      type="number"
                      step="0.5"
                      disabled={botState.isRunning}
                      value={credibilityInput}
                      onChange={e => {
                        setCredibilityInput(e.target.value);
                        const v = parseFloat(e.target.value);
                        if (!isNaN(v) && v >= 50 && v <= 80) {
                          onUpdateConfig({ credibilityFalloutPercent: v });
                        }
                      }}
                      onBlur={() => {
                        const v = parseFloat(credibilityInput);
                        const c = isNaN(v) || v < 50 || v > 80 ? 55 : v;
                        setCredibilityInput(c.toString());
                        onUpdateConfig({ credibilityFalloutPercent: c });
                      }}
                      className="w-full bg-slate-950 border border-slate-850 rounded-lg py-1.5 px-2.5 text-xs font-mono text-slate-100 focus:border-indigo-500 outline-none transition-colors disabled:opacity-45 pl-2.5 pr-8"
                    />
                    <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] font-mono text-slate-500 font-bold">%</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1 text-left">
              ...
              <div className="grid grid-cols-3 gap-1.55">
                {[
                  ...
                  <button
                    key={p.label}
                    onClick={() => onUpdateConfig(p.mult ? { stake: p.stake, martingaleMultiplier: p.mult } : { stake: p.stake })}
                    disabled={botState.isRunning}
                    title={`${p.label}: $${p.stake.toFixed(2)}${p.mult ? ` / ${p.mult}x` : ' / flat'}`}
          ...
          <div className="bg-slate-950/20 border border-slate-800/40 p-3 rounded-xl flex flex-col justify-between space-y-3.5">
            ...
            {/* Daily P&L Tracker */}
            {(botConfig.dailyStopLoss > 0 || botConfig.dailyTakeProfit > 0 || activeBalance > 0) && (
              <div className="space-y-1.5 pt-1 text-left">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">DAILY P&L</span>
                  <span className={`text-[9.5px] font-mono font-extrabold px-1.5 py-0.5 rounded border ${
                    botState.dailyPnL >= 0
                      ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/25'
                      : 'bg-rose-500/10 text-rose-400 border-rose-500/25'
                  }`}>
                    {botState.dailyPnL >= 0 ? '+' : ''}{(botState.dailyPnL ?? 0).toFixed(2)}
                  </span>
                </div>

                {/* Take-profit progress */}
                {botConfig.dailyTakeProfit > 0 && (
                  <div>
                    <div className="flex justify-between items-center text-[9.5px] font-mono mb-0.5 leading-none">
                      <span className="text-slate-500 uppercase font-bold">Take-Profit</span>
                      <span className="font-extrabold text-emerald-450">
                        {(botState.dailyPnL ?? 0).toFixed(2)} / {botConfig.dailyTakeProfit.toFixed(2)}
                      </span>
                    </div>
                    <div className="h-0.5 bg-slate-950 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${botState.dailyPnL >= botConfig.dailyTakeProfit ? 'bg-emerald-400' : 'bg-emerald-500/60'}`}
                        style={{ width: `${Math.min(100, ((botState.dailyPnL ?? 0) / botConfig.dailyTakeProfit) * 100)}%` }} />
                    </div>
                  </div>
                )}

                {/* Stop-loss progress */}
                {botConfig.dailyStopLoss > 0 && (
                  <div>
                    <div className="flex justify-between items-center text-[9.5px] font-mono mb-0.5 leading-none">
                      <span className="text-slate-500 uppercase font-bold">Stop-Loss</span>
                      <span className="font-extrabold text-rose-450">
                        {Math.abs(botState.dailyPnL ?? 0).toFixed(2)} / {botConfig.dailyStopLoss.toFixed(2)}
                      </span>
                    </div>
                    <div className="h-0.5 bg-slate-950 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-500 ${Math.abs(botState.dailyPnL) >= botConfig.dailyStopLoss ? 'bg-rose-400' : 'bg-rose-500/60'}`}
                        style={{ width: `${Math.min(100, (Math.abs(botState.dailyPnL ?? 0) / botConfig.dailyStopLoss) * 100)}%` }} />
                    </div>
                  </div>
                )}

                {/* Default 10% limits when no explicit limits set */}
                {!botConfig.dailyTakeProfit && !botConfig.dailyStopLoss && activeBalance > 0 && (
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <div className="flex justify-between items-center text-[9.5px] font-mono mb-0.5 leading-none">
                        <span className="text-slate-500 uppercase font-bold">TP 10%</span>
                        <span className="font-extrabold text-emerald-450">${(activeBalance * 0.10).toFixed(2)}</span>
                      </div>
                      <div className="h-0.5 bg-slate-950 rounded-full overflow-hidden">
                        <div className="h-full bg-emerald-500/60 rounded-full transition-all duration-500"
                          style={{ width: `${Math.min(100, ((botState.dailyPnL ?? 0) / (activeBalance * 0.10)) * 100)}%` }} />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between items-center text-[9.5px] font-mono mb-0.5 leading-none">
                        <span className="text-slate-500 uppercase font-bold">SL 10%</span>
                        <span className="font-extrabold text-rose-450">${(activeBalance * 0.10).toFixed(2)}</span>
                      </div>
                      <div className="h-0.5 bg-slate-950 rounded-full overflow-hidden">
                        <div className="h-full bg-rose-500/60 rounded-full transition-all duration-500"
                          style={{ width: `${Math.min(100, (Math.abs(botState.dailyPnL ?? 0) / (activeBalance * 0.10)) * 100)}%` }} />
                      </div>
                    </div>
                  </div>
                )}
