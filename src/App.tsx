...
import { 
  ...
  Zap, ShieldAlert, Sparkles, CircleAlert, RefreshCw, Coins, X,
  ChevronUp, Activity
} from 'lucide-react';
...
export default function App() {
  ...
  const [premiumLocked, setPremiumLocked] = useState<boolean>(true);
  const [autopilotPastTrades, setAutopilotPastTrades] = useState<PastTrade[]>([]);
  const [manualPastTrades, setManualPastTrades] = useState<PastTrade[]>([]);
  ...
  const insufficientBalanceShownRef = useRef<boolean>(false);
  const closingInsufficientBalanceRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    const handlePoll = async () => {
      try {
        ...
        if (data.premiumLocked !== undefined) setPremiumLocked(data.premiumLocked);
        setAutopilotPastTrades(data.autopilotPastTrades || []);
        setManualPastTrades(data.manualPastTrades || []);
        ...
        // 4b. Insufficient balance popup
        if (data.botState.status === 'insufficient_balance' && !insufficientBalanceShownRef.current && !closingInsufficientBalanceRef.current) {
          console.log('💰 INSUFFICIENT BALANCE DETECTED — showing top-up popup');
          insufficientBalanceShownRef.current = true;
          triggerPushNotification('💰 Insufficient Balance', 'Not enough funds to trade. Please top up your account.');
          setTimeout(() => setInsufficientBalanceOpen(true), 100);
        }

        // Reset insufficient balance refs once status moves away (user started bot again)
        if (data.botState.status !== 'insufficient_balance') {
          insufficientBalanceShownRef.current = false;
          closingInsufficientBalanceRef.current = false;
        }
        ...
        if (data.botState.status === 'paused_low_winrate' && !pausedForResumeRef.current) {
          ...
          const currentLimit = botConfig.credibilityFalloutPercent || 55;
          triggerPushNotification('🔄 Pair Swap Triggered', `Active pair dropped below ${currentLimit}% win rate. Scanning for best replacement...`);
  ...
  const handleRestartScanning = async () => {
    ...
    try {
      ...
      // Server resets all state including recentSignalExitDigits and
      // force-reconnects the Deriv WebSocket. The 1-second poll will
      // pick up the fresh zeroed state automatically within 1-2 seconds.
  ...
  return (
    <div className={`min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white selection:font-bold transition-colors duration-300 overflow-x-hidden max-w-full ${theme}`}>
          ...
          <div className="flex-1 px-4 pt-4 pb-40 md:px-8 space-y-4">
            ...
            {activeTab === 'intelligence' && (
              <div className="space-y-4">
                ...
                {intelligenceSubTab === 'leaderboard' && (
                  <LeaderboardTab botConfig={botConfig}
                ...
                {intelligenceSubTab === 'scanner' && (
                  <ScannerTab botConfig={botConfig}
                    symbolsState={symbolsState}
                    onSelectSymbolForTrading={handleSelectSymbolForTrading}
                    activeTradingSymbolId={botState.symbol}
                    botRunning={botState.isRunning}
            ...
            {activeTab === 'premium' && (
              <PremiumTab
                ...
                sessionUptime={sessionUptime}
                pastTrades={pastTrades}
                autopilotPastTrades={autopilotPastTrades}
      ...
      <SessionCompleteModal
        ...
        isAutopilot={(() => { const s = localStorage.getItem('mamba_premium_status'); return !!s && s !== 'idle'; })()}
        onClose={async () => {
      ...
      <InsufficientBalanceModal
        ...
        onClose={async () => {
          closingInsufficientBalanceRef.current = true;
          setInsufficientBalanceOpen(false);
          const userId = sessionUserId;
          // Reset bot to idle so the user can start a fresh run after topping up
          try { await fetch('/api/new-session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tgUserId: userId }) }); } catch(e) {}
        }}
      />
    </div>
